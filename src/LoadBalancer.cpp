...
/**
 * @brief Constructs the LoadBalancer with an initial number of servers.
...
/**
 * @brief Distributes a request to a server using round-robin scheduling.
...
/**
 * @brief Runs one simulation cycle: processes requests and adjusts server count.
 * @param curr_cycle The current clock cycle.
 */
void LoadBalancer::run_cycle(int curr_cycle) {
    ...
    if (total_reqs > overload_thresh) {
        std::cout << "Overloaded, adding new server." << std::endl;
...
/**
 * @brief Calculates the total number of queued requests on all servers.
 * @return Total queued requests count.
 */
